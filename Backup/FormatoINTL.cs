...
namespace ZConsultaBuro
{
    ...
    class FormatoINTL
    {
		...
		/*
		public void setDataTableEncabezadoIntl(DataTable tableEncabezadoIntl)
		{
			this.dataTableEncabezadoIntl = tableEncabezadoIntl;
		}

		public void setDataTableSegmentoPN(DataTable dataTableSegmentoPN)
		{
			this.dataTableSegmentoPN = dataTableSegmentoPN;
		}

		public void setDataTableSegmentoPA(DataTable dataTableSegmentoPA)
		{
			this.dataTableSegmentoPA = dataTableSegmentoPA;
		}*/
		#endregion
        ...
        public string EncabezadoINTL ()
        {
				...
				foreach(DataColumn dataColumn in dataTableEncabezadoIntl.Columns)
				{
					...
					switch(columna)
					{
						...
						case "ID_BURO": 
							...
							break;
						case "CLAVE_USUARIO": 
							claveUsuario  = this.lineaformatoIntlEncabezado(41,valor,10);
							break;
						case "PASS_USUARIO": 
							passUsuario  = this.lineaformatoIntlEncabezado(51,valor,8);
							break;
						case "TIPO_CONSULTA": 
			...
			#region codigo comentado para evaluar su eliminacion
			/*encabezadoINTL+= etiqueta  ;
			encabezadoINTL+= version  ;
			encabezadoINTL+= RefOperador  ;
			encabezadoINTL+= producto  ;
			encabezadoINTL+= pais  ;
			encabezadoINTL+= buroID  ;
			encabezadoINTL+= claveUsuario  ;
			encabezadoINTL+= passUsuario  ;
			encabezadoINTL+= tipoConsulta  ;
			encabezadoINTL+= tipoContrato  ;
			encabezadoINTL+= moneda  ;
			encabezadoINTL+= importeContrato  ;
			encabezadoINTL+= idioma  ;
			encabezadoINTL+= tipoSalida  ;
			encabezadoINTL+= bloaqueRespuesta  ;
			encabezadoINTL+= impresoraID  ;
			encabezadoINTL+= usoFuturo  ;*/
			//encabezadoINTL= ubicacion.ToString();
			//string encabezadoEnOrden = new string(ubicacion);
			#endregion
				...
				foreach(DataColumn dataColumn in dataTableSegmentoPN.Columns)
				{
					...
					switch(columna)
					{
						...
						case "APE_MATERNO": 
							...
							break;
						case "APE_ADICIONAL": 
							apellidoAdicional = this.lineaformatoIntl("00",valor,26);
							break;
						case "PRIMER_NOMBRE": 
							primerNombre = this.lineaformatoIntl("02", valor ,26);
							break;
						case "SEGUNDO_NOMBRE": 
							apellidoAdicional = this.lineaformatoIntl("03",valor,26);
							break;
						...
						case "FECHA_NACIMIENTO": 
							...
							break;
						case "RFC": 
								if(valor.Length==10){NumRFC = this.lineaformatoIntl("05", valor.Substring(0,10) ,10);}
								if(valor.Length==13){NumRFC = this.lineaformatoIntl("05", valor.Substring(0,13) ,13);}
								if(valor.Length==0){NumRFC = this.lineaformatoIntl("05", valor,13);}
							break;
						case "PREFIJO": 
						...
						case "SUFIJO": 
							...
							break;
						case "NACIONALIDAD": 
							nacionalidad  = this.lineaformatoIntl("08",valor,1);
							break;
						case "RESIDENCIA": 
							residencia  = this.lineaformatoIntl("09",valor,2);
							break;
						case "LIC_CONDUCIR": 
		...
		public string SegmentoDeCierre(string total)
		{
			...
			//int largo = total.Length;
			//byte[] data = System.Text.Encoding.GetBytes(total);
			return total;
		}
		#endregion
